...
import { THERMLGRAM_PATHS as THERMOGRAM_PATHS } from '../../node/mocks/thermogram.mock';
...
describe("ThermalRegistry", () => {
    ...
    test("loading of files", async () => {
        ...
        expect(group.instances.value.length).toEqual(1);
        ...
        expect(registry.groups.addOrGetGroup(GROUP_ID).hash).toEqual(group.hash);
        expect(group.instances.value.length).toEqual(2);
