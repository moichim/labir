...
export default function Edit({ attributes, setAttributes }) {
	...
	const {
		url, subfolder, palette, author, license, label, enablegrouping
	} = attributes;
	...
	const getRequestUrl = useCallback((url, subfolder) => {
			...
			<InspectorControls>
				...
				<PanelBody title="Display settings">
					...
					<ToggleControl
						...
						help={
							enablegrouping
								? 'Can display multiple folders at once. Folders shall be grouped by time.'
								: 'Can only see one folder at a time.'
						}
						checked={enablegrouping}
						onChange={(newValue) => {
							setAttributes({ enablegrouping: newValue });
						}}
					/>
